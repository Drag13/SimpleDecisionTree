export function calculateEntropy(dataSet: string[], value: string) {
  ...
  const valueEntropy =
    (givenLabelLength / allLabelsLength) *
    Math.log2(givenLabelLength / allLabelsLength);
  const otherEntropy =
    (otherLabelsQuantity / allLabelsLength) *
    Math.log2(otherLabelsQuantity / allLabelsLength);
